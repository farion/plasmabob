...
pub(crate) fn detect_player_collectibles(
    ...
    mut player_query: Query<
        (
            ...
            &mut crate::game::components::health::Health,
        ),
        With<crate::game::components::player::Player>,
    >,
            ...
            if collectible_query.get(colliding_entity).is_ok() {
                if let Ok(effect) = effect_heal_query.get(colliding_entity) {
                    ...
                    player_health.current =
                        (player_health.current + heal_amount).min(player_health.max);
                    info!(
                        "Player healed by {} - HP: {}/{}",
                        heal_amount, player_health.current, player_health.max
                    );
                    commands
                        .entity(_player_entity)
                        .insert(health_floating::RecentHealthChange(heal_amount));
                }
