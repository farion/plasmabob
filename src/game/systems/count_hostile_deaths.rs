...
pub(crate) fn count_hostile_deaths(
    ...
    dead_hostiles: Query<(Entity, &crate::game::components::health::Health), (With<Hostile>, With<SpawnedLevelEntity>, Without<crate::game::systems::combat_types::DeathCounted>)>,
) {
    for (entity, health) in &dead_hostiles {
        if !health.is_dead() { continue; }
        stats.enemies_killed = stats.enemies_killed.saturating_add(1);
        commands.entity(entity).insert(crate::game::systems::combat_types::DeathCounted);
    }
}
