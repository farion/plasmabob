...
pub fn render_level_picker_columns(
    ui: &mut egui::Ui,
    open_asset_path: &mut Option<String>,
    catalog: &mut crate::level::state::LevelCatalog,
    sync_state: &mut crate::level::state::EntityTypesSyncState,
    ...
    ui.columns(3, |columns| {
        columns[0].vertical(|ui| {
            ...
            if ui.button("Force Reread").clicked() {
                if let Ok(worlds) = crate::core::io::scan_worlds() {
                    catalog.worlds = worlds;
                }
                if let Ok(levels) = crate::core::io::scan_levels() {
            ...
            if ui.button("Regenerate entity types").clicked() {
                ...
                if !sync_state.running.load(std::sync::atomic::Ordering::SeqCst) {
                    ...
                    std::thread::spawn(move || {
                        let res = crate::core::io::sync_entity_types_with_sprites();
