...
pub(crate) fn render_json_property(
    ...
    let mut text_value = entity_type_editor.json_editor_state.get(editor_key).cloned().unwrap_or(initial_text.to_string());
    let response = ui.add(
    ...
    if response.changed() {
        entity_type_editor.json_editor_state.insert(editor_key.to_string(), text_value.clone());
