...
mod array_editor;
use array_editor::{ArrayEditorState, format_array_short, inner_array_value_to_csv_string, parse_array_type_signature};
mod components_sidebar;
use components_sidebar::render_components_sidebar as sidebar_render_components_sidebar;
mod helpers;
use helpers::{AttributeUiRow, cloned_staged_entity_type, apply_to_staged_entity_type, component_object_snapshot, component_default_value, save_staged_entity_type, sorted_attribute_rows};

mod hitbox;
use hitbox::{EntityTypeEditorState, RectHitbox, ActiveHitboxDrag, DragEdge, hitbox_to_screen, units_per_pixel, hitbox_to_screen_with_ratio, pick_hitbox_edge, cursor_for_drag_edge};
mod number_property;
mod string_property;
mod bool_property;
mod enum_property;
mod json_property;
use number_property::render_number_property;
use string_property::render_string_property;
use bool_property::render_bool_property;
use enum_property::render_enum_property;
use json_property::render_json_property;
mod array_property;
use array_property::render_array_property;
...
/// Shows: components, width/height and per-state animation images.
 pub(crate) fn entity_type_view_ui(
    ...
    let dirty_mark = if entity_type_editor.dirty_entity_types.contains(&selected_name) { " *" } else { "" };
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if ui.button(egui_phosphor_icons::icons::X).clicked() {
                    ...
                    if entity_type_editor.dirty_entity_types.contains(&selected_name) {
    ...
    if ctx.input(|input| input.modifiers.ctrl && input.key_pressed(egui::Key::S)) {
        ...
        if !entity_type_editor.dirty_entity_types.contains(&selected_name) && entity_type_editor.dirty_states.is_empty() {
                                    ...
                                    if entity_type_editor.show_hitboxes {
                                        ...
                                        if response.drag_started() {
                                            if let Some(pointer_pos) = response.interact_pointer_pos() {
                                                if let Some(edge) = pick_hitbox_edge(pointer_pos, screen_hitbox) {
                                                    entity_type_editor.active_drag = Some(ActiveHitboxDrag {
                                        ...
                                        if response.dragged() {
                                            if let Some(active_drag) = &entity_type_editor.active_drag {
                                                if active_drag.state_key == state_key {
                                                    ...
                                                    entity_type_editor.dirty_states.insert(state_key.clone());
                                                    // Mark the whole entity type as dirty so the title shows '*' and
                                                    // Ctrl+S will save staged changes.
                                                    entity_type_editor.dirty_entity_types.insert(selected_name.clone());
                                                }
                                            }
                                        }

                                        entity_type_editor.edited_hitboxes.insert(state_key.clone(), rect);
                                    }
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.button("Discard and Close").clicked() {
                        ...
                        entity_type_editor.edited_entity_types.remove(&selected_name);
                        entity_type_editor.dirty_entity_types.remove(&selected_name);
                        entity_type_editor.dirty_states.clear();
                        *show_close_confirm = false;
                        next_state.set(crate::editor::EditorMode::LevelPicker);
